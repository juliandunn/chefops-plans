...
do_install() {
  ...
  for i in bsondump mongodump mongoimport mongorestore mongotop mongo mongoexport mongooplog mongos mongod mongofiles mongoperf mongostat; do
    patchelf --interpreter "$(pkg_path_for glibc)/lib/ld-linux-x86-64.so.2" ${pkg_prefix}/bin/$i
  done
}
